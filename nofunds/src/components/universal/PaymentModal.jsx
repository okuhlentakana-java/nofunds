...
export default function PaymentModal({ isOpen, onClose, item, onSuccess }) {
  ...
  // Stay mounted to show result even after onClose() hides the parent
  ...
  const buildFullPhone = (raw) => {
    const digits = raw.replace(/\D/g, "");
    return `${LESOTHO.dial}${digits}`;
  };
  ...
  body: JSON.stringify({
    ...
    email:    email.trim() || undefined,
        ...
        // Close our modal so PayFast has the full screen
        onClose();

        window.payfast_do_onsite_payment({ uuid: data.uuid }, (result) => {
          console.log("PayFast callback result:", result, typeof result);
          ...
          <div className="p-5">
            ...
            {localDigits.length > 0 && (
              <p className="text-xs text-gray-400 mb-1">
                Full number: <span className="font-bold text-indigo-600 font-mono">{buildFullPhone(localPhone)}</span>
              </p>
            )}

            {/* Email — card only, optional */}
            {isCard && (
              <>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 mb-2 flex items-start gap-2">
                  ...
                  <p className="text-xs text-blue-700">
                    {email.trim()
                      ...
                      : "Add an email above to receive a receipt."}
                  </p>
                </div>
              </>
            )}
